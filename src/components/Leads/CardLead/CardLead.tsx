...
const CardLead = ({ lead }: LeadProps) => {
  ...
  const status_eng = statusNames[lead.status_id][0];
  const status_ru = statusNames[lead.status_id][1];

  const borderColor = colorStyles['border_status_' + status_eng];
    ...
    <div className={styles.card + ' ' + borderColor}>
          ...
          <p>
            {lead.completed_at &&
              new Intl.DateTimeFormat('ru-RU', {
                ...
                hour: '2-digit',
                minute: '2-digit',
              }).format(new Date(lead.completed_at))}
          </p>
        </div>
        <div className={styles.text_elem}>
          <span>Оплачена:</span>
          <p>
            {lead.payment_at &&
              new Intl.DateTimeFormat('ru-RU', {
                ...
                hour: '2-digit',
                minute: '2-digit',
              }).format(new Date(lead.payment_at))}
          </p>
        </div>
      </div>
      ...
      <div className={styles.prices}>
        <div className={styles.services}>
          <div className={styles.service}>
            <svg
              width="800px"
              height="800px"
              className={lead.is_internet ? color : colorStyles.color_status_default}
              viewBox="0 0 24 24"
              fill="none"
            ...
            <span>{lead.reward_internet ? lead.reward_internet + '₽' : ''}</span>
          </div>
          <div className={styles.service}>
            <svg version="1.1" className={lead.is_cleaning ? color : colorStyles.color_status_default} id="Icons" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
              <g fill="currentColor">
		...
		s1,0.4,1,1v3h4c0.3,0,0.6-0.1,0.8-0.4c0.2-0.2,0.3-0.5,0.2-0.8L25.1,22H6.9z"
            ...
            <span>{lead.reward_cleaning ? lead.reward_cleaning + '₽' : ''}</span>
          </div>
          <div className={styles.service}>
            <svg
              width="800px"
              height="800px"
              className={lead.is_shipping ? color : colorStyles.color_status_default}
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
            ...
            <span>{lead.reward_shipping ? lead.reward_shipping + '₽' : ''}</span>
          </div>
        </div>

        <span className={styles.repayment}>{lead.reward_internet + lead.reward_cleaning + lead.reward_shipping + '₽'}</span>
      </div>

      <span className={styles.status + ' ' + backgroundColor}>{status_ru}</span>
    </div>
  );
};
